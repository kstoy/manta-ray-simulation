...
# Label the x and y axes; keep the panes/grid hidden and blank the z-axis.
LABEL_FS = 22
TICK_FS = 18
ax.set_xlabel("x", fontsize=LABEL_FS)
ax.set_ylabel("y", fontsize=LABEL_FS)
ax.set_xticks([0, D_RODS])
ax.set_yticks([0, D_RODS])
ax.set_zticks([z_lo, z_hi])
ax.set_zticklabels([f"{z_lo:.1f}", f"{z_hi:.1f}"])
...
for ext in ("png", "pdf"):
    out_path = Path(__file__).parent / f"cell_catenaries.{ext}"
